...
const handleData = async (stateData) => {
    ...
    const newCases = await fetch(getEndpointNewCases(encodeURI(stateData.LAN_ew_GEN)))
    .then((res) => res.json())
    .then((_json) => _json.features[0].attributes.value).catch((error) => {
      console.log('\x1b[31m%s\x1b[0m', ` x Error fetching handleData: fetch(getEndpointNewCases)`);
      console.log(error);
    });

    const newDeaths = await fetch(getEndpointNewDeaths(encodeURI(stateData.LAN_ew_GEN)))
    .then((res) => res.json())
    .then((_json) => _json.features[0].attributes.value).catch((error) => {
      console.log('\x1b[31m%s\x1b[0m', ` x Error fetching handleData: fetch(getEndpointNewDeaths)`);
      console.log(error);
    });
    ...
    stateData.last_update = `${moment(stateData.Aktualisierung).format('DD.MM.YYYY, HH:mm')} Uhr`; 
